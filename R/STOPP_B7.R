#' Evaluates the imported patients' data for the STOPP B7 criterion.
#'
...
STOPP_B7 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
         ...
         )
      {
        ...
        if ( ( any(grepl('^C03C|^C03EB', unlist(pdata[[i]][1]), ignore.case=T)) # checking primary condition C03C* OR C03EB* in the med_gen_decod list
                ...
                any(grepl('I87.2|^R60', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I87.2 OR R60* in the ih_icd_10_decod list
                any(grepl('I87.2|^R60', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition I87.2 OR R60* in the h_icd_10_decod list
              )
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP B7: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B7: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B7.xlsx'), col_names = TRUE)
  }
