...
STOPP_D3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('N05AA01|N05AH02|N05AF01|N05AB02|N05AC04|N05AA03|N05AF05', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition N05AA01 OR N05AH02 OR N05AF01 OR N05AB02 OR N05AC04 OR N05AA03 OR N05AF05 in the med_gen_decod list
           ( any(grepl('N40|R33', unlist(pdata[[i]][2]), ignore.case=T)) | # checking for secondary conditions N40 OR R33 in the ih_icd10__decod list.
             any(grepl('N40|R33', unlist(pdata[[i]][3]), ignore.case=T)) # checking for secondary conditions N40 OR R33 in the h_icd10__decod list.
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP D3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
