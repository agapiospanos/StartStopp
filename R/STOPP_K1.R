#' Evaluates the imported patients' data for the STOPP K1 criterion.
#'
...
STOPP_K1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){ # checking if missing_data_patients contain pid
      ...
      if ( any(grepl('^N05BA|^N05CD|^N03AE', unlist(pdata[[i]][1]), ignore.case=T))) { # checking primary condition N05BA* OR N05CD* OR N03AE* in the med_gen__decod list
  ...
  if (suppressNA) {
    cat('STOPP K1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP K1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-K1.xlsx'), col_names = TRUE)
  }
