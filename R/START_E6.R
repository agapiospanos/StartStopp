#' Evaluates the imported patients' data for the START E6 criterion.
#'
...
START_E6 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^M04AA', unlist(pdata[[i]][1]), ignore.case = T)) |       # checking without condition M01AA* in the med_gen__decod list
         ...
         )
      {
        ...
        if ( any(grepl('^M10', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition M10* in the ih_icd10_decod list
             any(grepl('^M10', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition M10* in the h_icd10_decod list
        )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START E6: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START E6: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-E6.xlsx'), col_names = TRUE)
  }
