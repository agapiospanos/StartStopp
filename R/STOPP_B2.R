#' Evaluates the imported patients' data for the STOPP B2 criterion.
#'
...
STOPP_B2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('C08DA01|C08DB01|C09BB10|C08DA51', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition C08DA01 OR C08DB01 OR C09BB10 OR C08DA51 in the med_gen_decod list
  ...
  if (suppressNA) {
    cat('STOPP B2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B2.xlsx'), col_names = TRUE)
  }
