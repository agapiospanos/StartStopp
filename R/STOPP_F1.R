#' Evaluates the imported patients' data for the STOPP F1 criterion.
#'
...
STOPP_F1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('A03FA01|N05AB04', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for A03FA01 OR N05AB04 in the med_gen_decod list
           ( any(grepl('G20|^G21|G23.1|G23.2|G31.8', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions G20 OR G21* OR G23.1 OR G23.2 OR G31.8 in the ih_icd10__decod list.
             any(grepl('G20|^G21|G23.1|G23.2|G31.8', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions G20 OR G21* OR G23.1 OR G23.2 OR G31.8 in the h_icd10__decod list.
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP F1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP F1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-F1.xlsx'), col_names = TRUE)
  }
