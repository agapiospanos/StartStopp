#' Evaluates the imported patients' data for the STOPP C10 criterion.
#'
...
STOPP_C10 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){ # checking if missing_data_patients contain pid
      ...
      if ( any(grepl('^M01A|^M01BA|^N02BA', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition M01A* OR M01BA* OR N02BA*
           any(grepl('^B01AA|^B01AE|^B01AF', unlist(pdata[[i]][1]), ignore.case=T)) # checking secondary condition B01AA* OR B01AE* OR B01AF*
      ) {
  ...
  if (suppressNA) {
    cat('STOPP C10: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP C10: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-C10.xlsx'), col_names = TRUE)
  }
