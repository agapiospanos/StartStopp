#' Evaluates the imported patients' data for the START C6 criterion.
#'
...
START_C6 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('N04BC04|N04BC05|N04BC09', unlist(pdata[[i]][1]), ignore.case=T)) | # checking without condition N04BC04 OR N04BC05 OR N04BC09 in the med_gen_decod list
           any(grepl('N18.4|N18.5|E61.1|D50|D50.8|D50.9', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition N18.4 OR N18.5 OR E61.1 OR D50 OR D50.8 OR D50.9 in the ih_icd10_decod list
           any(grepl('N18.4|N18.5|E61.1|D50|D50.8|D50.9', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition N18.4 OR N18.5 OR E61.1 OR D50 OR D50.8 OR D50.9 in the h_icd10_decod list
      ){
        ...
        if ( any(grepl('G25.8', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition G25.8 in the ih_icd10_decod list
             any(grepl('G25.8', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition G25.8 in the h_icd10_decod list
            ){
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START C6: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START C6: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-C6.xlsx'), col_names = TRUE)
  }
