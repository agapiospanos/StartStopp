#' Evaluates the imported patients' data for the STOPP E2 criterion.
#'
...
STOPP_E2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^B01AE', unlist(pdata[[i]][1]), ignore.case=T)) & ( cp_egrf_cond | any(grepl('N18.4|N18.5', unlist(pdata[[i]][3]), ignore.case=T)) | any(grepl('N18.4|N18.5', unlist(pdata[[i]][4]), ignore.case=T)) ) ) {
  ...
  if (suppressNA) {
    cat('STOPP E2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP E2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-E2.xlsx'), col_names = TRUE)
  }
