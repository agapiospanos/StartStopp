#' Evaluates the imported patients' data for the START G1 criterion.
#'
...
START_G1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
         ...
         )
      {
        ...
        if (any(is.na(gender))) {
          ...
          dm_sex_cond <- gender == 'male'   # checking condition dm_sex = male
        }
  ...
  if (suppressNA) {
    cat('START G1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START G1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-G1.xlsx'), col_names = TRUE)
  }
