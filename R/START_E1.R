#' Evaluates the imported patients' data for the START E1 criterion.
#'
...
START_E1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('L04AX01|L04AX03|L04AA13|L04AD01|A07EC01|^P01BA|^M01CB|^M01CC|L01BA01|^L04AB|L04AA24|L04AC03|L04AC07|L04AC10|L01XC02|L04AA13', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition L04AX01 OR L04AX03 OR L04AA13 OR L04AD01 OR A07EC01 OR P01BA* OR M01CB* OR M01CC* OR L01BA01 OR L04AB* OR L04AA24 OR L04AC03 OR L04AC07 OR L04AC10 OR L01XC02 OR L04AA13 in the med_gen_decod list
      ){
        ...
        if ( any(grepl('^M05|^M06', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition M05* OR M06* in the ih_icd10_decod list
             any(grepl('^M05|^M06', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition M05* OR M06* in the h_icd10_decod list
            ){
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START E1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START E1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-E1.xlsx'), col_names = TRUE)
  }
