...
STOPP_H3 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('N02BE01|N02BE51|N02BE71', unlist(pdata[[i]][1]), ignore.case=T)) # checking unless condition N02BE01 OR N02BE51 OR N02BE71 in the med_gen_decod list
        ...
        if ( ( any(grepl('^M01A|^N02BA|^M01BA', unlist(pdata[[i]][1]), ignore.case=T)) # checking primary condition M01A* OR N02BA* OR M01BA*  in the med_gen_decod list
          ...
          any(grepl('^M15|^M16|^M17|^M18|^M19|^M47', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition M15* OR M16* OR M17* OR M18* OR M19* OR M47* in the ih_icd_10_decod list
          any(grepl('^M15|^M16|^M17|^M18|^M19|^M47', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition M15* OR M16* OR M17* OR M18* OR M19* OR M47* in the h_icd_10_decod list
        )
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  cat ('STOPP H3: ', fullfil_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
