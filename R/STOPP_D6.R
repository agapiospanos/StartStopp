...
STOPP_D6 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
      ...
      if ( any(grepl('N05AH04|N05AH02|^N05AN', unlist(pdata[[i]][1]), ignore.case=T)) # checking unless condition N05AH04 OR N05AH02 OR N05AN* in the med_gen__decod list
        ...
        if ( any(grepl('^N05A', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition N05A* in the med_gen_decod list
             ( any(grepl('G20|^G21|G23.1|G23.2|G31.8|G90.3', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition G20 OR G21* OR G23.1 OR G23.2 OR G31.8 OR G90.3 in the ih_icd10__decod list
               any(grepl('G20|^G21|G23.1|G23.2|G31.8|G90.3', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition G20 OR G21* OR G23.1 OR G23.2 OR G31.8 OR G90.3 in the h_icd10__decod list
             )
           )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP D6: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D6: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
