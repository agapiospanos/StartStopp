#' Evaluates the imported patients' data for the STOPP D2 criterion.
#'
...
STOPP_D2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^N06AB|^N06AX|N06CA03', unlist(pdata[[i]][1]), ignore.case=T)) # checking unless condition N06AB* OR N06AX* OR N06CA03 in the med_gen__decod list
      ){
        ...
        if ( any(grepl('^N06AA|N06CA01|N06CA02', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition N06AA* OR N06CA01 OR N06CA02 in the med_gen_decod list
             ( any(grepl('^F32|^F33', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition F32* OR F33* in the ih_icd10__decod list
               any(grepl('^F32|^F33', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition F32* OR F33* in the h_icd10__decod list
             )
           )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP D2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-D2.xlsx'), col_names = TRUE)
  }
