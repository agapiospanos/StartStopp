#' Evaluates the imported patients' data for the STOPP H8 criterion.
#'
...
STOPP_H8 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^A02BC', unlist(pdata[[i]][1]), ignore.case=T))) { # checking for A02BC* in the med_gen_decod list
        ...
        if ( any(grepl('^M01BA', unlist(pdata[[i]][1]), ignore.case=T)) | # checking for M01BA* in the med_gen_decod list
             (
               any(grepl('^M01A|^N02BA|N02CB01|H02BX01|M01AX01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for M01A* OR N02BA* OR N02CB01 OR H02BX01 OR M01AX01 in the med_gen_decod list
               any(grepl('^H02A|^H02B|N02CB01', unlist(pdata[[i]][1]), ignore.case=T))  # checking for H02A* OR H02B* OR N02CB01 in the med_gen_decod list
             )
        )
        {
  ...
  if (suppressNA) {
    cat('STOPP H8: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP H8: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-H8.xlsx'), col_names = TRUE)
  }
