#' Evaluates the imported patients' data for the STOPP G2 criterion.
#'
...
STOPP_G2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      # checking for the unless conditions
      if ( any(grepl('^R03BA', unlist(pdata[[i]][1]), ignore.case=T))) { # checking for R03BA* in the med_gen_decod list
        ...
        if ( any(grepl('^H02AB|H02BX01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for H02AB* OR H02BX01 in the med_gen_decod list
             (
               any(grepl('J40|^J41|J42|^J43|^J44', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions J40 OR J41* OR J42 OR J43* OR J44* in the ih_icd10__decod list.
               any(grepl('J40|^J41|J42|^J43|^J44', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions J40 OR J41* OR J42 OR J43* OR J44* in the h_icd10__decod list.
             )
        )
        {
  ...
  if (suppressNA) {
    cat('STOPP G2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP G2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-G2.xlsx'), col_names = TRUE)
  }
