...
STOPP_E3 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^B01AF', unlist(pdata[[i]][1]), ignore.case=T)) & (as.numeric(unlist(pdata[[i]][2])) < 15 | any(grepl('N18.5', unlist(pdata[[i]][3]), ignore.case=T))) ) {
  ...
  cat ('STOPP E3: ', fulfill_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
