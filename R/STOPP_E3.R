#' Evaluates the imported patients' data for the STOPP E3 criterion.
#'
...
STOPP_E3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if (length(cp_egrf) > 0) {
        cp_egrf_cond <- any(cp_egrf < 15)
      }

      # checking if fulfills at least one set of primary condition AND secondary condition
      if ( any(grepl('^B01AF', unlist(pdata[[i]][1]), ignore.case=T)) & ( cp_egrf_cond | any(grepl('N18.5', unlist(pdata[[i]][3]), ignore.case=T)) | any(grepl('N18.5', unlist(pdata[[i]][4]), ignore.case=T)) ) ) {
  ...
  if (suppressNA) {
    cat('STOPP E3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP E3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-E3.xlsx'), col_names = TRUE)
  }
