#' Evaluates the imported patients' data for the STOPP L2 criterion.
#'
...
STOPP_L2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  data <- import_excel_data(current_data = data, path = path, worksheet = 1, var_col = 'med_interv', include_missing = suppressNA, ignore_na = suppressNA )
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if (any(grepl('^A06|A02AA04|N02AA55', unlist(pdata[[i]][1]), ignore.case=T))) {
        ...
        # get vectors of atc codes and medicine interval for the current patient
        patient_atc_codes <- unlist(pdata[[i]][1])
        med_interv <- unlist(pdata[[i]][2])

        cond1 <- cond2 <- cond3 <- cond4 <- cond5 <- FALSE

        index1 <- grep('^N02A', patient_atc_codes, ignore.case = T)
        if (length(index1) > 0) { # we get length of index because the grep returns an empty integer vector if this atc code is not found.
          ...
          if (any(!is.na(med_interv1))) {
            cond1 <- any(grepl('^day$|^week$|^year$|^every other day$|^every other week$', med_interv1, ignore.case = T))
          }
        }

        index2 <- grep('R05DA04', patient_atc_codes, ignore.case = T)
        if (length(index2) > 0) { # we get length of index because the grep returns an empty integer vector if this atc code is not found.
          ...
          if (any(!is.na(med_interv2))) {
            cond2 <- any(grepl('^day$|^week$|^year$|^every other day$|^every other week$', med_interv2, ignore.case = T))
          }
        }

        index3 <- grep('N07BC06', patient_atc_codes, ignore.case = T)
        if (length(index3) > 0) { # we get length of index because the grep returns an empty integer vector if this atc code is not found.
          ...
          if (any(!is.na(med_interv3))) {
            cond3 <- any(grepl('^day$|^week$|^year$|^every other day$|^every other week$', med_interv3, ignore.case = T))
          }
        }

        index4 <- grep('N07BC02', patient_atc_codes, ignore.case = T)
        if (length(index4) > 0) { # we get length of index because the grep returns an empty integer vector if this atc code is not found.
          ...
          if (any(!is.na(med_interv4))) {
            cond4 <- any(grepl('^day$|^week$|^year$|^every other day$|^every other week$', med_interv4, ignore.case = T))
          }
        }

        index5 <- grep('N07BC01', patient_atc_codes, ignore.case = T)
        if (length(index5) > 0) { # we get length of index because the grep returns an empty integer vector if this atc code is not found.
          ...
          if (any(!is.na(med_interv5))) {
            cond5 <- any(grepl('^day$|^week$|^year$|^every other day$|^every other week$', med_interv5, ignore.case = T))
          }
        }
  ...
  if (suppressNA) {
    cat('STOPP L2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP L2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-L2.xlsx'), col_names = TRUE)
  }
