#' Evaluates the imported patients' data for the STOPP B3 criterion.
#'
...
STOPP_B3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^C07', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition C07* in the med_gen_decod list
           any(grepl('C08DA01|C08DB01|C09BB10|C08DA51', unlist(pdata[[i]][1]), ignore.case=T)) # checking secondary condition C08DA01 OR C08DB01 OR C09BB10 OR C08DA51 in the med_gen_decod list
      ) {
  ...
  if (suppressNA) {
    cat('STOPP B3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B3.xlsx'), col_names = TRUE)
  }
