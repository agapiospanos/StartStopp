#' Evaluates the imported patients' data for the STOPP D13 criterion.
#'
...
STOPP_D13 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){ # checking if missing_data_patients contain pid
      ...
      if ( any(grepl('G20|^G21|G23.1|G23.2|G31.8|G90.3', unlist(pdata[[i]][2]), ignore.case=T)) |
           any(grepl('G20|^G21|G23.1|G23.2|G31.8|G90.3', unlist(pdata[[i]][3]), ignore.case=T)) ) {
        ...
        if ( any(grepl('^N04B', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary conditions in the med_gen__decod list
             (
               any(grepl('G25.0', unlist(pdata[[i]][2]), ignore.case=T)) |
               any(grepl('G25.0', unlist(pdata[[i]][3]), ignore.case=T))
             )
           )
        {
  ...
  if (suppressNA) {
    cat('STOPP D13: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D13: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-D13.xlsx'), col_names = TRUE)
  }
