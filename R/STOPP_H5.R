#' Evaluates the imported patients' data for the STOPP H5 criterion.
#'
...
STOPP_H5 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^H02AB|H02BX01|^M01BA', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition H02AB* OR H02BX01 OR M01BA* in the med_gen_decod list
           ( any(grepl('^M15|^M16|^M17|^M18|^M19|^M47', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition M15* OR M16* OR M17* OR M18* OR M19* OR M47* in the ih_icd10__decod list
             any(grepl('^M15|^M16|^M17|^M18|^M19|^M47', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition M15* OR M16* OR M17* OR M18* OR M19* OR M47* in the h_icd10__decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP H5: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP H5: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-H5.xlsx'), col_names = TRUE)
  }
