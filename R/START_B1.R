#' Evaluates the imported patients' data for the START B1 criterion.
#'
...
START_B1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('R03AK04|R03AK05|R03AK06|R03AK07|R03AK08|R03AK09|R03AK10|R03AK11|R03AK12|R03AK13|^R03AL|^R03AC|^R03BB', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition R03AK04 OR R03AK05 OR R03AK06 OR R03AK07 OR R03AK08 OR R03AK09 OR R03AK10 OR R03AK11 OR R03AK12 OR R03AK13 OR R03AL* OR R03AC* OR R03BB* in the med_gen_decod list
      ){
        ...
        if ( any(grepl('J40|^J41|J42|^J43|^J44|^J45|J46', unlist(pdata[[i]][2]), ignore.case=T)) |   # checking primary condition J40 OR J41* OR J42 OR J43* OR J44* OR J45* OR J46 in the ih_icd10_decod list
  ...
  if (suppressNA) {
    cat('START B1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START B1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-B1.xlsx'), col_names = TRUE)
  }
