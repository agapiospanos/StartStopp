#' Evaluates the imported patients' data for the STOPP B10 criterion.
#'
...
STOPP_B10 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){ # checking if missing_data_patients contain pid
      ...
      if ( any(grepl('^C02AB|^C02AC|^C02LC|^C02LB', unlist(pdata[[i]][1]), ignore.case=T))) { # checking primary condition C02AB* OR C02AC* OR C02LC* OR C02LB*
  ...
  if (suppressNA) {
    cat('STOPP B10: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B10: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B10.xlsx'), col_names = TRUE)
  }
