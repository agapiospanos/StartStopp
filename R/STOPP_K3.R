...
STOPP_K3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^G04CA|^C02CA|C02LE01|^C07AG|^C07BG|^C07CG|^C08|^C09|^C01DA|C01DX12', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition G04CA* OR C02CA* OR C02LE01 OR C07AG* C07BG* OR C07CG* OR C08* OR C09* OR C01DA* OR C01DX12 in the med_gen_decod list
           ( any(grepl('I95.1', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I95.1 in the ih_icd10__decod list
             any(grepl('I95.1', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition I95.1 in the h_icd10__decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP K3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP K3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
