#' Evaluates the imported patients' data for the STOPP G3 criterion.
#'
...
STOPP_G3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^R03BB|R03AL', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for R03BB* OR R03AL in the med_gen_decod list
           ( any(grepl('H40.2|N40|R33', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions H40.2 OR N40 OR R33 in the ih_icd10__decod list.
             any(grepl('H40.2|N40|R33', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions H40.2 OR N40 OR R33 in the h_icd10__decod list.
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP G3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP G3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-G3.xlsx'), col_names = TRUE)
  }
