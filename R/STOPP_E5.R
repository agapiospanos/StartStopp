#' Evaluates the imported patients' data for the STOPP E5 criterion.
#'
...
STOPP_E5 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if (length(cp_egrf) > 0) {
        cp_egrf_cond <- any(cp_egrf < 10)
      }

      # checking if fulfills at least one set of primary condition AND secondary condition
      if ( any(grepl('M04AC01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for M04AC01 in the med_gen_decod list
           ( cp_egrf_cond | # checking if cp_egrf is less than 10
             ( any(grepl('N18.5', unlist(pdata[[i]][3]), ignore.case=T)) | # checking the secondary conditions N18.5 in the ih_icd10__decod list.
               any(grepl('N18.5', unlist(pdata[[i]][4]), ignore.case=T))   # checking the secondary conditions N18.5 in the h_icd10__decod list.
             )
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP E5: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP E5: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-E5.xlsx'), col_names = TRUE)
  }
