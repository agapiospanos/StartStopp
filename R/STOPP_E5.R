...
STOPP_E5 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('M04AC01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for M04AC01 in the med_gen_decod list
           ( as.numeric(unlist(pdata[[i]][2])) < 10 | # checking if cp_egrf is less than 10
  ...
  cat ('STOPP E5: ', fulfill_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
