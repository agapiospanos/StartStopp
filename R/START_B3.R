#' Evaluates the imported patients' data for the START B3 criterion.
#'
...
START_B3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  data <- import_excel_data(current_data = data, path = path, worksheet = 4, var_col = 'cp_os', include_missing = suppressNA, ignore_na = suppressNA )
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('V03AN01', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition V03AN01 in the med_gen_decod list
      ){
      ...
      # checking if fulfills at least one set of primary condition AND secondary condition
        ...
        if (
             ...
             any(grepl('R09.0|J95.1|J95.2|J95.3|^J96', unlist(pdata[[i]][6]), ignore.case=T)) | # checking for primary conditions R09.0 OR J95.1 OR J95.2 OR J95.3 OR J96* in the h_icd10__decod list
             cp_po_cond |
             cp_os_cond
           )
        {
  ...
  if (suppressNA) {
    cat('START B3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START B3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-B3.xlsx'), col_names = TRUE)
  }
