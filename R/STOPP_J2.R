#' Evaluates the imported patients' data for the STOPP J2 criterion.
#'
...
STOPP_J2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^A10BG|A10BD04|A10BD03|A10BD06|A10BD06|A10BD05|A10BD09|A10BD12', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition A10BG* OR A10BD04 OR A10BD03 OR A10BD06 OR A10BD06 OR A10BD05 OR A10BD09 OR A10BD12 in the med_gen_decod list
           ( any(grepl('^I50|I11.0|I13.0|I13.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I50* OR I11.0 OR I13.0 OR I13.2 in the ih_icd10__decod list
             any(grepl('^I50|I11.0|I13.0|I13.2', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition I50* OR I11.0 OR I13.0 OR I13.2 in the h_icd10__decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP J2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP J2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-J2.xlsx'), col_names = TRUE)
  }
