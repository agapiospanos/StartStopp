...
STOPP_E1 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if (length(index1)>0) { # we get length of index because the grep returns an empty integer vector if the C01AA05 is not found.
        if (as.numeric(daily_dosage[index1]) > 0.125) { # checking if daily_dosage for this atc code is greater that 0.125
          cond1 <- TRUE
        }
      }

      # checking if fulfills at least one set of primary condition AND secondary condition
      if ( cond1 & (as.numeric(unlist(pdata[[i]][3])) < 30 | any(grepl('N18.4|N18.5', unlist(pdata[[i]][4]), ignore.case=T))) ) {
  ...
  cat ('STOPP E1: ', fulfill_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
