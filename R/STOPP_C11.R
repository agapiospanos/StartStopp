#' Evaluates the imported patients' data for the STOPP C11 criterion.
#'
...
STOPP_C11 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^A02BC', unlist(pdata[[i]][1]), ignore.case=T)) # checking unless condition A02BC** in the med_gen__decod list
        ...
        if ( any(grepl('^M01A|^M01BA|^N02BA', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition M01A* OR M01BA* OR N02BA* in the med_gen_decod list
             any(grepl('^B01AC', unlist(pdata[[i]][1]), ignore.case=T)) # checking secondary condition B01AC* in the med_gen__decod list
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP C11: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP C11: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-C11.xlsx'), col_names = TRUE)
  }
