#' Evaluates the imported patients' data for the START F1 criterion.
#'
...
START_F1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^C09A|^C09B|^C09C|^C09D', unlist(pdata[[i]][1]), ignore.case=T)) ){
        ...
        if (
              (
                any(grepl('^E10|^E11|^E12|^E13|^E14', unlist(pdata[[i]][2]), ignore.case=T)) | # checking primary condition E10* OR E11* OR E12* OR E13* OR E14* in the ih_icd10__decod list
                any(grepl('^E10|^E11|^E12|^E13|^E14', unlist(pdata[[i]][3]), ignore.case=T))   # checking primary condition E10* OR E11* OR E12* OR E13* OR E14* in the h_icd10__decod list
              ) & (
                any(grepl('R80|E10.2|E11.2|E12.2|E13.2|E14.2|N08.3', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition R80 OR E10.2 OR E11.2 OR E12.2 OR E13.2 OR E14.2 OR N08.3 in the ih_icd10__decod list
                any(grepl('R80|E10.2|E11.2|E12.2|E13.2|E14.2|N08.3', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition R80 OR E10.2 OR E11.2 OR E12.2 OR E13.2 OR E14.2 OR N08.3 in the h_icd10__decod list
              )
           )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START F1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START F1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-F1.xlsx'), col_names = TRUE)
  }
