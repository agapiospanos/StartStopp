...
STOPP_E4 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^M01A|M01AX01|M01BA01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for M01A* OR M01AX01 OR M01BA01 in the med_gen_decod list
           ( cp_egrf_cond | # checking if cp_egrf is less than 50
             ( any(grepl('N18.3|N18.4|N18.5', unlist(pdata[[i]][3]), ignore.case=T)) | # checking the secondary conditions N18.3 OR N18.4 OR N18.5 in the ih_icd10__decod list.
               any(grepl('N18.3|N18.4|N18.5', unlist(pdata[[i]][4]), ignore.case=T)) # checking the secondary conditions N18.3 OR N18.4 OR N18.5 in the h_icd10__decod list.
             )
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP E4: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP E4: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
