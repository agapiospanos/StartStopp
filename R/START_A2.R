#' Evaluates the imported patients' data for the START A2 criterion.
#'
...
START_A2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('B01AC06|B01AC08|B01AC08|^B01AA|^B01AE|^B01AF|^B01AB', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition B01AC06 OR B01AC08 OR B01AC56 OR B01AA* OR B01AE* OR B01AF* OR B01AB* in the med_gen_decod list
      ){
        ...
        if ( any(grepl('I48.2', unlist(pdata[[i]][2]), ignore.case=T)) |   # checking primary condition I48.2 in the ih_icd10_decod list
             any(grepl('I48.2', unlist(pdata[[i]][3]), ignore.case=T)) ) { # checking primary condition I48.2 in the h_icd10_decod list
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START A2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START A2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-A2.xlsx'), col_names = TRUE)
  }
