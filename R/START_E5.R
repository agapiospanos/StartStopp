#' Evaluates the imported patients' data for the START E5 criterion.
#'
...
START_E5 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^A11CC|^A11CB|^A12AX|M05BB03|M05BB04|M05BB05|M05BB06|M05BB07|M05BB08', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition A11CC* OR A11CB* OR A12AX* OR M05BB03 OR M05BB04 OR M05BB05 OR M05BB06 OR M05BB07 OR M05BB08 in the med_gen_decod list
      ){
        ...
        if ( any(grepl('R29.6|R26.3|W01|W05|W06|W07|W08|W10|W18|W19', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition R29.6 OR R26.3 OR W01 OR W05 OR W06 OR W07 OR W08 OR W10 OR W18 OR W19 in the ih_icd10_decod list
             any(grepl('R29.6|R26.3|W01|W05|W06|W07|W08|W10|W18|W19', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition R29.6 OR R26.3 OR W01 OR W05 OR W06 OR W07 OR W08 OR W10 OR W18 OR W19 in the h_icd10_decod list
            )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START E5: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START E5: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-E5.xlsx'), col_names = TRUE)
  }
