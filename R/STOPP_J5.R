#' Evaluates the imported patients' data for the STOPP J5 criterion.
#'
...
STOPP_J5 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^G03AC|^G03D|^L02AB', unlist(pdata[[i]][1]), ignore.case=T)) | # checking for G03AC* OR G03D* OR L02AB* in the med_gen_decod list
           any(grepl('Z90.7', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions Z90.7 in the ih_icd10__decod list.
           any(grepl('Z90.7', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions Z90.7 in the h_icd10__decod list.
         )
      {
        ...
        if ( any(grepl('^G03C|^G03E|^G03HB|^G03XC|^L02AA', unlist(pdata[[i]][1]), ignore.case=T))) { # checking for G03C* OR G03E* OR G03HB* OR G03XC* OR L02AA* in the med_gen_decod list
  ...
  if (suppressNA) {
    cat('STOPP J5: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP J5: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-J5.xlsx'), col_names = TRUE)
  }
