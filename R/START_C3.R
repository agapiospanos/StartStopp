#' Evaluates the imported patients' data for the START C3 criterion.
#'
...
START_C3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if (
            (
              (
                any(grepl('^F00|^G30', unlist(pdata[[i]][2]), ignore.case=T)) | # checking primary condition F00* OR G30* in the ih_icd10_decod list
                any(grepl('^F00|^G30', unlist(pdata[[i]][3]), ignore.case=T))   # checking primary condition F00* OR G30* in the h_icd10_decod list
              ) & !(
                any(grepl('^N06DA', unlist(pdata[[i]][1]), ignore.case=T))      # checking primary condition N06DA* in the med_gen__decod list
              )
            ) | (
              any(grepl('G31.8', unlist(pdata[[i]][2]), ignore.case=T)) |       # checking primary condition G31.8 in the ih_icd10_decod list
              any(grepl('G31.8', unlist(pdata[[i]][3]), ignore.case=T))         # checking primary condition G31.8 in the h_icd10_decod list
            ) & !(
              any(grepl('N06DA03', unlist(pdata[[i]][1]), ignore.case=T))       # checking primary condition N06DA03 in the med_gen__decod list
            )
         )
      {
  ...
  if (suppressNA) {
    cat('START C3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START C3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-C3.xlsx'), col_names = TRUE)
  }
