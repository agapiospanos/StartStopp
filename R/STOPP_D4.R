#' Evaluates the imported patients' data for the STOPP D4 criterion.
#'
...
STOPP_D4 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){ # checking if missing_data_patients contain pid
      ...
      if ( any(grepl('^N06BA|N06CA03', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary conditions in the med_gen__decod list
           (
             any(grepl('E87.1', unlist(pdata[[i]][2]), ignore.case=T)) |
             any(grepl('E87.1', unlist(pdata[[i]][3]), ignore.case=T)) |
             cp_sod_cond
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP D4: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D4: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-D4.xlsx'), col_names = TRUE)
  }
