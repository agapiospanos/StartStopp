#' Evaluates the imported patients' data for the START I2 criterion.
#'
...
START_I2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  # Importing the data
  data <- import_excel_data(path = path, worksheet = 5, var_col = 'mh_pneu_vacc', include_missing = suppressNA, ignore_na = suppressNA ) # 0 = no, 1 = yes, 99 = unknown
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( as.numeric(unlist(pdata[[i]][1])) == 99 | as.numeric(unlist(pdata[[i]][1])) == 1 )
      {
        ...
        if ( as.numeric(unlist(pdata[[i]][1])) == 0 ) # checking primary conditions
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START I2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START I2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-I2.xlsx'), col_names = TRUE)
  }
