...
STOPP_H9 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^M05BA|^M05BB', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition M05BA* OR M05BB* in the med_gen_decod list
           ( any(grepl('K20|^K21|^K22|^K23|K25|K26|K27|K28|^K29|K30|^K31|R10.1|R12|R13|K92.0|K92.1|K92.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition K20 OR K21* OR K22* OR K23* OR K25 OR K26 OR K27 OR K28 OR K29* OR K30 OR K31* OR R10.1 OR R12 OR R13 OR K92.0 OR K92.1 OR K92.2 in the ih_icd10__decod list
             any(grepl('K20|^K21|^K22|^K23|K25|K26|K27|K28|^K29|K30|^K31|R10.1|R12|R13|K92.0|K92.1|K92.2', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition K20 OR K21* OR K22* OR K23* OR K25 OR K26 OR K27 OR K28 OR K29* OR K30 OR K31* OR R10.1 OR R12 OR R13 OR K92.0 OR K92.1 OR K92.2 in the h_icd10__decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP H9: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP H9: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
