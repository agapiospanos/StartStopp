#' Evaluates the imported patients' data for the START A7 criterion.
#'
...
START_A7 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^C07', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition C07* in the med_gen_decod list
        ...
        if ( any(grepl('^I20|^I21|^I22|^I24|^I25|Z95.5|Z95.1', unlist(pdata[[i]][2]), ignore.case=T)) |   # checking primary condition I20* OR I21* OR I22* OR I24* OR I25* OR Z95.5 OR Z95.1 in the ih_icd10_decod list
             any(grepl('^I20|^I21|^I22|^I24|^I25|Z95.5|Z95.1', unlist(pdata[[i]][3]), ignore.case=T)) ) { # checking primary condition I20* OR I21* OR I22* OR I24* OR I25* OR Z95.5 OR Z95.1 in the h_icd10_decod list
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START A7: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START A7: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-A7.xlsx'), col_names = TRUE)
  }
