...
STOPP_D9 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
         ...
         )
      {
        ...
        if ( any(grepl('^N05A|^N06CA', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition N05A* OR N06CA* in the med_gen_decod list
             ( any(grepl('^F00|^F01|^F02|F03|^F07|^G30|G31.0|G31.1|G31.8', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition F00* OR F01* OR F02* OR F03 OR F07* OR G30* OR G31.0 OR G31.1 OR G31.8 in the ih_icd10__decod list
               any(grepl('^F00|^F01|^F02|F03|^F07|^G30|G31.0|G31.1|G31.8', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition F00* OR F01* OR F02* OR F03 OR F07* OR G30* OR G31.0 OR G31.1 OR G31.8 in the h_icd10__decod list
             )
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP D9: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP D9: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }
