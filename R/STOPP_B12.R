#' Evaluates the imported patients' data for the STOPP B12 criterion.
#'
...
STOPP_B12 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  data <- import_excel_data(current_data = data, path = path, worksheet = 4, var_col = 'cp_pot_date', ignore_na = TRUE, include_missing = TRUE)
  ...
  if (suppressNA) {
    cat('STOPP B12: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B12: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B12.xlsx'), col_names = TRUE)
  }
