#' Evaluates the imported patients' data for the STOPP B13 criterion.
#'
...
STOPP_B13 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('G04BE03|G04BE08|G04BE09|G04BE10', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition G04BE03 OR G04BE08 OR G04BE09 OR G04BE10 in the med_gen_decod list
           ( any(grepl('^I55|I11.0|I13.0|I13.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I55* OR I11.0 OR I13.0 OR I13.2 in the ih_icd10__decod list
             any(grepl('^I55|I11.0|I13.0|I13.2', unlist(pdata[[i]][3]), ignore.case=T)) | # checking secondary condition I55* OR I11.0 OR I13.0 OR I13.2 in the h_icd10__decod list
             any(grepl('^C01DA', unlist(pdata[[i]][1]), ignore.case=T)) # checking secondary condition C01DA* in the med_gen_decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP B13: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B13: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B13.xlsx'), col_names = TRUE)
  }
