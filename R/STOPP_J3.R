#' Evaluates the imported patients' data for the STOPP J3 criterion.
#'
...
STOPP_J3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^C07', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for C07* in the med_gen_decod list
           (
             any(grepl('E10|E11|E12|E13|E14', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions E10 OR E11 OR E12 OR E13 OR E14 in the ih_icd10__decod list.
             any(grepl('E10|E11|E12|E13|E14', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions E10 OR E11 OR E12 OR E13 OR E14 in the h_icd10__decod list.
           ) &
           (
             any(grepl('E16.0|E16.1|E16.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking the secondary conditions E16.0 OR E16.1 OR E16.2 in the ih_icd10__decod list.
             any(grepl('E16.0|E16.1|E16.2', unlist(pdata[[i]][3]), ignore.case=T))   # checking the secondary conditions E16.0 OR E16.1 OR E16.2 in the h_icd10__decod list.
           )
         )
      {
  ...
  if (suppressNA) {
    cat('STOPP J3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP J3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-J3.xlsx'), col_names = TRUE)
  }
