#' Evaluates the imported patients' data for the START A1 criterion.
#'
...
START_A1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^B01AA|^B01AE|^B01AF', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition B01AA* OR B01AE* OR B01AF* in the med_gen_decod list
      ){
  ...
  if (suppressNA) {
    cat('START A1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START A1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-A1.xlsx'), col_names = TRUE)
  }
