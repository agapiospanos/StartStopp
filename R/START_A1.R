...
START_A1 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
  ...
  # Importing the data
  data <- import_excel_data(path = path, worksheet = 2, var_col = 'ih_icd10__decod')
  data <- import_excel_data(current_data = data, path = path, worksheet = 1, var_col = 'med_gen__decod')
  ...
  cat ('START A1: ', fulfill_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
