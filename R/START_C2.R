#' Evaluates the imported patients' data for the START C2 criterion.
#'
...
START_C2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^N06AB|^N06AF|^N06AG|^N06AX|N06CA03', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition N06AB* OR N06AF* OR N06AG* OR N06AX* OR N06CA03 in the med_gen_decod list
      ){
        ...
        if ( any(grepl('^F32|^F33', unlist(pdata[[i]][2]), ignore.case=T)) |  # checking primary condition F32* OR F33* in the ih_icd10_decod list
             any(grepl('^F32|^F33', unlist(pdata[[i]][3]), ignore.case=T))    # checking primary condition F32* OR F33* in the h_icd10_decod list
            ){
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START C2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START C2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-C2.xlsx'), col_names = TRUE)
  }
