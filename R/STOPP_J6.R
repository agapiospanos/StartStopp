#' Evaluates the imported patients' data for the STOPP J6 criterion.
#'
...
STOPP_J6 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('E29.1|E89.5|Q98.0|Q98.1|Q98.2|Q98.4', unlist(pdata[[i]][2]), ignore.case=T)) | # checking unless condition E29.1 OR E89.5 OR Q98.0 OR Q98.1 OR Q98.2 OR Q98.4 in the ih_icd10__decod list
           any(grepl('E29.1|E89.5|Q98.0|Q98.1|Q98.2|Q98.4', unlist(pdata[[i]][3]), ignore.case=T))   # checking unless condition E29.1 OR E89.5 OR Q98.0 OR Q98.1 OR Q98.2 OR Q98.4 in the h_icd10__decod list
      ){
        ...
        if ( any(grepl('^G03B', unlist(pdata[[i]][1]), ignore.case=T)) # checking primary condition G03B* in the med_gen_decod list
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP J6: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP J6: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-J6.xlsx'), col_names = TRUE)
  }
