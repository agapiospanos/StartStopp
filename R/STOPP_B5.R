#' Evaluates the imported patients' data for the STOPP B5 criterion.
#'
...
STOPP_B5 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('C01BD01', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition C01BD01 in the med_gen_decod list
           ( any(grepl('I47.1|^I48', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I47.1 OR I48* in the ih_icd10__decod list
             any(grepl('I47.1|^I48', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition I47.1 OR I48* in the h_icd10__decod list
           )
      ) {
  ...
  if (suppressNA) {
    cat('STOPP B5: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B5: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B5.xlsx'), col_names = TRUE)
  }
