#' Evaluates the imported patients' data for the STOPP C6 criterion.
#'
...
STOPP_C6 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( years_interval_cond1 | years_interval_cond2 ) {
        ...
        if ( any(grepl('^B01AC', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition B01AC* in the med_gen_decod list
  ...
  if (suppressNA) {
    cat('STOPP C6: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP C6: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-C6.xlsx'), col_names = TRUE)
  }
