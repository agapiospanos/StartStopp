#' Evaluates the imported patients' data for the STOPP C4 criterion.
#'
...
STOPP_C4 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      # we don't have to get the patient_fup2m_date[index1] because fup2m_dates are not related to the ATC codes
      ...
      if (length(index1)>0) { # we get length of index because the grep returns an empty integer vector if the Z95.5 is not found.
        ...
        icd_10_dates_single <- unlist(patient_icd10_dates[index1])
        icd_10_dates_single <- icd_10_dates_single[!is.na(icd_10_dates_single)]

        if (length(icd_10_dates_single) > 0 & length(patient_fup2m_dates) > 0) {

          years_interval <- as.numeric(as.period(interval(as.POSIXct(icd_10_dates_single, format = "%m/%d/%Y", origin = as.POSIXct("1970/1/1")), as.POSIXct(patient_fup2m_dates, format = "%m/%d/%Y", origin = as.POSIXct("1970/1/1")))), "years")
      ...
      if (length(index2)>0) { # we get length of index because the grep returns an empty integer vector if the Z95.5 is not found.
        ...
        icd_10_dates_single <- unlist(patient_icd10_dates[index2])
        icd_10_dates_single <- icd_10_dates_single[!is.na(icd_10_dates_single)]
      ...
      if ( years_interval_cond1 | years_interval_cond2 ) {
        ...
        if ( any(grepl('B01AC06|B01AC08|B01AC56|B01AC30', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition B01AC06 OR B01AC08 OR B01AC56 OR B01AC30 in the med_gen_decod list
             any(grepl('B01AC04', unlist(pdata[[i]][1]), ignore.case=T)) & # checking for primary condition B01AC04 in the med_gen__decod list.
             ( any(grepl('^I63|^I64|^G45', unlist(pdata[[i]][2]), ignore.case=T)) | # checking for secondary conditions I63* OR I64* OR G45* in the ih_icd10__decod list.
               any(grepl('^I63|^I64|^G45', unlist(pdata[[i]][4]), ignore.case=T))   # checking for secondary conditions I63* OR I64* OR G45* in the h_icd10__decod list.
             )
        )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP C4: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP C4: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-C4.xlsx'), col_names = TRUE)
  }
