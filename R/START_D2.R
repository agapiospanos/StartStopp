#' Evaluates the imported patients' data for the START D2 criterion.
#'
...
START_D2 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^A06AC', unlist(pdata[[i]][1]), ignore.case=T)) # checking without condition A06AC* in the med_gen_decod list
      ){
        ...
        if (
              (
                any(grepl('^K57', unlist(pdata[[i]][2]), ignore.case=T)) | # checking primary condition K57* in the ih_icd10_decod list
                any(grepl('^K57', unlist(pdata[[i]][3]), ignore.case=T))   # checking primary condition K57* in the h_icd10_decod list
              ) & (
                any(grepl('K59.0', unlist(pdata[[i]][2]), ignore.case=T)) | # checking primary condition K59.0 in the ih_icd10_decod list
                any(grepl('K59.0', unlist(pdata[[i]][3]), ignore.case=T))   # checking primary condition K59.0 in the h_icd10_decod list
              )
           )
        {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('START D2: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START D2: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-D2.xlsx'), col_names = TRUE)
  }
