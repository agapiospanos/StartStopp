...
STOPP_B1 <- function(path, excel_out = TRUE, export_data_path=getwd()) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^I48', unlist(pdata[[i]][2]), ignore.case=T)) # checking unless condition I48* in the ih_icd_10_decod list
      ){
  ...
  cat ('STOPP B1: ', fulfill_count, 'patients out of', total_count, 'patients fulfill the criterion.', missing_count, 'patients have missing data. \n')
