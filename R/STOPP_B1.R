#' Evaluates the imported patients' data for the STOPP B1 criterion.
#'
...
STOPP_B1 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('^I48', unlist(pdata[[i]][2]), ignore.case=T)) | any(grepl('^I48', unlist(pdata[[i]][3]), ignore.case=T)) # checking unless condition I48* in the ih_icd_10_decod and h_icd_10_decod list
      ){
        ...
        if ( any(grepl('C01AA05|C01AA08', unlist(pdata[[i]][1]), ignore.case=T)) & # checking primary condition C01AA05 OR C01AA08 in the med_gen_decod list
             ( any(grepl('^I50|I11.0|I13.0|I13.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking secondary condition I50* OR I11.0 OR I13.0 OR I13.2 in the ih_icd_10_decod list
               any(grepl('^I50|I11.0|I13.0|I13.2', unlist(pdata[[i]][3]), ignore.case=T))   # checking secondary condition I50* OR I11.0 OR I13.0 OR I13.2 in the h_icd_10_decod list
             )
        ) {
          # inserting the record to the data.frame evaluated_patients
  ...
  if (suppressNA) {
    cat('STOPP B1: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('STOPP B1: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/STOPP-B1.xlsx'), col_names = TRUE)
  }
