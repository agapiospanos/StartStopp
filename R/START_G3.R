#' Evaluates the imported patients' data for the START G3 criterion.
#'
...
START_G3 <- function(path = NULL, excel_out = TRUE, export_data_path = NULL, suppressNA = TRUE) {
  ...
  for ( i in 1: length(pdata)){
    ...
    if (is.na(match( pid, names(sapply(missing_data_patients, names))))){
      ...
      if ( any(grepl('G03CA03|G03CA04|G03CA57', unlist(pdata[[i]][3]), ignore.case=T)) ) # checking without conditions in the med_gen__decod list
      {
        ...
        if (
              (
                any(grepl('N95.2', unlist(pdata[[i]][2]), ignore.case=T)) | # checking primary condition N95.2 in the ih_icd10__decod list
                any(grepl('N95.2', unlist(pdata[[i]][3]), ignore.case=T))   # checking primary condition N95.2 in the h_icd10__decod list
              ) & (
                dm_sex_cond
              )
           )
  ...
  if (suppressNA) {
    cat('START G3: ', fulfill_count, 'patients out of', total_count + missing_count, 'patients meet the criterion.\n')
  } else {
    cat('START G3: ', fulfill_count, 'patients out of', total_count, 'patients meet the criterion.', missing_count, 'patients have missing data. \n')
  }

  if (excel_out) {
    # export the evaluated list of patients to excel file
    write_xlsx(evaluated_patients, path = paste0( export_data_path, '/START-G3.xlsx'), col_names = TRUE)
  }
